...
export const calculateIron = selectedFoods$ => {
  const ironNutrients = JSON.parse(JSON.stringify(defaultIron))

  selectedFoods$.subscribe(
    selectedFood => {
      selectedFood.amount = selectedFood.amount || 0
      ironNutrients.iron.raw += selectedFood.food.nutrients.Iron * selectedFood.amount
      // console.log(selectedFood.amount)
      // console.log(ironNutrients.iron.raw)
    },
    err => console.error(err),
    () => {
      ironNutrients.iron = formatIronNutrient(ironNutrients.iron, ['μg'])
    },
  )

  // console.log(ironNutrients.iron.raw)
  return ironNutrients
}

const formatIronNutrient = (ironNutrient, units) =>
  Object.assign(ironNutrient, unitize(ironNutrient.raw, units))
